#include "WindowImplWin32.hpp" // included first to avoid a warning about macro redefinition
...
////////////////////////////////////////////////////////////
WglContext::WglContext(WglContext* shared, const ContextSettings& settings, const WindowImplWin32* owner, unsigned int bitsPerPixel) :
m_window       (NULL),
m_deviceContext(NULL),
m_context      (NULL),
m_ownsWindow   (false)
...
void WglContext::createContext(WglContext* shared, unsigned int bitsPerPixel, const ContextSettings& settings)
{
    ...
    while (!m_context && (m_settings.majorVersion >= 3))
    {
        ...
        if (wglCreateContextAttribsARB)
        {
            ...
            int attributes[] =
            {
                ...
                WGL_CONTEXT_PROFILE_MASK_ARB, profile, 0, 0
            };
    ...
    if (!m_context)
    {
        ...
        if (sharedContext)
        {
			...
			wglShareLists(sharedContext, m_context);
        }
    }
}
