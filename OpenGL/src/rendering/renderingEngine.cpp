...
RenderingEngine::RenderingEngine(const Window& window) :
	...
	m_window(&window),
	m_tempTarget(window.getSize().x, window.getSize().y, 0, GL_TEXTURE_2D, GL_NEAREST, GL_RGBA, GL_RGBA, false, GL_COLOR_ATTACHMENT0),
	...
	m_altCamera(Matrix4f().InitIdentity(), &m_altCameraTransform)
{
	...
	SetTexture("displayTexture", Texture(window.getSize().x, window.getSize().y, 0, GL_TEXTURE_2D, GL_LINEAR, GL_RGBA, GL_RGBA, true, GL_COLOR_ATTACHMENT0));
...
void RenderingEngine::ProcessInput(const Event& event)
{
	if (event.type == Event::Resized)
	{
		SetTexture("displayTexture", Texture(event.size.width,event.size.height, 0, GL_TEXTURE_2D, GL_LINEAR, GL_RGBA, GL_RGBA, true, GL_COLOR_ATTACHMENT0));
	}
}
...
void RenderingEngine::clear(float r, float g, float b, float a)
{
...
//		glScissor(0, 0, 100, 100);
	...
	float displayTextureAspect = GetTexture("displayTexture").GetAspect();
